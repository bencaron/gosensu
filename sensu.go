...
func New(name string, path string, url string, timeout int, username string, password string, token string, insecure bool) *Sensu {
	tr := &http.Transport{
	...
	return &Sensu{name, path, url, timeout, username, password, token, client}
}
